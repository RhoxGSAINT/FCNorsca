...
local mission_to_next={
    hkrul_birna_turn_four_incident = function() 
...
local function birna_bogtus_listeners()
	...
	if sarl_faction_interface and sarl_faction_interface:is_human() then
		...
		core:add_listener(
			"rhox_birna_turn2_start",
			"FactionTurnStart",
			function(context)
				local faction = context:faction()
				
				return faction:name() == sarl_faction and faction:is_human() and cm:model():turn_number() == 2
			end,
			function(context)
				local faction = context:faction()
				local incident_builder = cm:create_incident_builder("hkrul_birna_turn_three_incident")
