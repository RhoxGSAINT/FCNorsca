...
local function rhox_sarg_set_diktat_listeners()
        
    --- Trigger check for visibility when settlement is selected
    core:add_listener(
        "rhox_sarg_diktat_trigger",
        "SettlementSelected",
        true,
        function(context)
    ...
    core:add_listener(
        "rhox_sarg_performed_slave_diktats",
        "ComponentLClickUp",
        function(context)
    ...
    core:add_listener(
        "rhox_sarg_diktat_click_trigger_1",
        "ComponentLClickUp",
        function (context)
    ...
    --- Trigger whenever vampire coven slot is pressed
    core:add_listener(
        "rhox_sarg_diktat_click_trigger_2",
        "ComponentLClickUp",
        function (context)
    ...
    core:add_listener(
        "rhox_sarg_diktat_click_trigger_3",
        "ComponentLClickUp",
        function (context)
...
local function rhox_sarg_god_bar_ui()
...
cm:add_first_tick_callback(
	function()
		if cm:get_local_faction_name(true) == sargath_faction then
            ...
            local result = core:get_or_create_component("rhox_sarg_devotee_holder", "ui/campaign ui/rhox_sarg_devotee_holder.twui.xml", parent_ui)
            rhox_sarg_set_diktat_listeners()
            
            
            core:add_listener(
                "rhox_sarg_disable_vanilla_occupation_options",
                "PanelOpenedCampaign",
                function(context)
            ...
            rhox_sarg_god_bar_ui()
            
            

            local parent_ui = find_uicomponent(core:get_ui_root(), "hud_campaign", "faction_buttons_docker", "button_group_management");
            local result = UIComponent(parent_ui:CreateComponent("rhox_sarg_chaos_gift", "ui/campaign ui/rhox_sarg_chaos_gift_button.twui.xml"))
            
            
            core:add_listener(
                "rhox_sarg_panel_open_button_leftclick",
                "ComponentLClickUp",
                function (context)
                    return context.string == "rhox_sarg_chaos_gift"
                end,
                function ()
                    
                    local panel = core:get_or_create_component("rhox_sarg_chaos_gift_panel", "ui/campaign ui/rhox_sarg_chaos_gift_panel.twui.xml", core:get_ui_root())
                end,
                true
            )
            ...
            core:add_listener(
                "rhox_sarg_panel_close_button_leftclick",
                "ComponentLClickUp",
                function (context)
                    return context.string == "rhox_sarg_button_ok"
                end,
                function ()
                    local panel = find_uicomponent(core:get_ui_root(), "rhox_sarg_chaos_gift_panel")
                    if panel then
                        panel:Destroy()
                    end
                end,
