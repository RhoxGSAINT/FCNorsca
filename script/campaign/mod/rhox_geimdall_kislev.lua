...
function rhox_bjornling_setup_kislev_devotion()
	...
	-------AI thing, now they don't have ritual so I'll just put some random value in it
	...
	core:add_listener(
		...
		"BuildingCompleted",
		function(context)
			return has_supporters_pooled_resource(context:building():faction()) and context:building():faction():is_human() == false and cm:model():random_percent(25)
		end,
		function(context)
			-- cache this value as buildings complete at end of round, so to display the pooled resource change in the UI, add it at the start of their turn instead
			local save_value = "followers_to_add_" .. context:building():faction():name()
	...
	core:add_listener(
		...
		"GarrisonOccupiedEvent",
		function(context)
            return has_supporters_pooled_resource(context:character():faction()) and context:character():faction():is_human() == false and cm:model():random_percent(25)
		end,
		function(context)
			add_followers(context:character():faction(), 5);
			check_followers()
	...
	core:add_listener(
		...
		function()
			...
			for i = 1, cm:pending_battle_cache_num_attackers() do
				...
				if attacker_faction and has_supporters_pooled_resource(attacker_faction) and cm:model():random_percent(25) then
					add_followers(attacker_faction, 2);
					followers_added = true;
				end
			end
			...
			for i = 1, cm:pending_battle_cache_num_defenders() do
				...
				if defender_faction and has_supporters_pooled_resource(defender_faction) and cm:model():random_percent(25) then
					add_followers(defender_faction, 2);
					followers_added = true;
				end
			end
