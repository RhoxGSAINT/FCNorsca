    ...
    wh_main_nor_baersonling ={
        ...
        additional = function(faction, faction_key)
            ...
            if faction:is_human() then
				...
				cm:trigger_mission(faction_key, "hkrul_baerson_einar_aesling_battle", true)
				--init incident
				local incident_builder = cm:create_incident_builder("rhox_baersonling_chain_0")
                core:add_listener(
                    "rhox_fc_norsca_baersonling init_incident",
                    "ScriptEventIntroCutsceneFinished",
                    true,
                    function(context)
                        cm:callback(function() 
...
cm:add_first_tick_callback_new(
    function()
        ...
        for faction_key, faction_info in pairs(rhox_faction_list) do
			local faction = cm:get_faction(faction_key);
			out("Rhox FC NORSCA: Currently looking at: "..faction_key)
